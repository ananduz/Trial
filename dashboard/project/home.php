...
$select_documents = $conn->prepare("SELECT * FROM `content` WHERE tutor_id = ?");
$select_documents->execute([$tutor_id]);
$total_documents = $select_documents->rowCount();

// Fetch student reviews
$select_reviews = $conn->prepare("SELECT * FROM `comments` WHERE tutor_id = ?");
$select_reviews->execute([$tutor_id]);
